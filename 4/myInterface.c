...
drawing_OP ()
{
  ...
  int command;
  int operand;
  sc_memoryGet (instruction_counter1, &value);
  mt_gotoXY (66, 8);
  char buf[32];
...
ui_Counter ()
{
  ...
  instruction_counter1 = atoi (buffer);
  if (instruction_counter1 < 0 || instruction_counter1 > 99)
    {
  ...
  drawing_IC (instruction_counter1);
  return 0;
}
...
drawing_IC ()
{
  mt_gotoXY (70, 5);
  char buff[32];
  sprintf (buff, "+%04d", instruction_counter1);
